...
module.exports = async (req, res) => {
  ...
  if (!user || !user.paid) return res.status(401).json({ error: 'Email ou mot de passe incorrect.' });
  ...
  if (device_id && !isAdmin) {
    ...
    if (!user.device_id) {
      await supabase.from('users').update({ device_id }).eq('email', email.toLowerCase().trim());
    }
  }

  // ── Token cryptographiquement sûr ────────────────────────────────────────
